...
public static class Tools {
    ...
    public static List<Rigidbody> GetRigidbodies(Collider[] cols) {
        ...
        foreach (Collider c in cols) {
            if (c != null) {
                Rigidbody rb = FindInParents<Rigidbody>(c.transform);
    ...
    public static int ColorToInt(Color c) {
        int retVal = 0;
        retVal |= Mathf.RoundToInt(c.r * 255f);
        retVal |= Mathf.RoundToInt(c.g * 255f) << 8;
        retVal |= Mathf.RoundToInt(c.b * 255f) << 16;
        retVal |= Mathf.RoundToInt(c.a * 255f) << 24;
        return retVal;
    }
