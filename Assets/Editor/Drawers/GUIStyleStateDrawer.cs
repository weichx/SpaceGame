...
public class GUISkinEditor : MultiWindowEditor {
    ...
    public void OnEnable() {
        ...
        _guiStyleAreaColor = new Color(194, 194, 194, 255);
    }
    ...
    protected override void OnMultiInspectorAuxGUI(Rect totalArea) {
        if (!showAux) return;
        GUILayout.BeginArea(totalArea);
        _auxScroll = GUILayout.BeginScrollView(_auxScroll);
        GUILayout.BeginVertical();
        GUILayout.BeginHorizontal();
        GUILayout.FlexibleSpace();
    ...
    void DrawBackgroundField(GUIStyleState state, string stateName) {
        EditorGUILayout.BeginHorizontal();
        state.background = ObjectField(state.background, stateName);
        if (state.background != null) {
            if (state.background.name.IndexOf("_bg_gen", StringComparison.Ordinal) != -1) {
                Color color = state.background.GetPixel(0, 0);
                Color newColor = EditorGUILayout.ColorField(color);
                state.background.SetColor(newColor);
                if (GUI.changed) {
                    Undo.RecordObject(_guiSkin, "GUISkin " + _guiSkin.name + " Changed");
                    EditorUtility.SetDirty(_guiSkin);
                }
            }
        ...
        if (GUILayout.Button("Create", GUILayout.Width(100))) {
            Texture2D textureAsset = new Texture2D(2, 2, TextureFormat.RGBA32, true);
            textureAsset.alphaIsTransparency = true;
            textureAsset.SetColor(new Color(1, 1, 1, 1));
            state.background = textureAsset;
            AssetDatabase.CreateAsset(textureAsset, $"Assets/Editor Default Resources/UI Textures/{_guiSkin.name}_{stateName.ToLower()}_bg_gen.asset");
        }
        EditorGUILayout.EndHorizontal();
    }
    ...
    void DrawGUIStyle(Style inStyle) {
        ...
        if ((inStyle.flags & ControlFlags.Normal) != 0) {
            DrawBackgroundField(style.normal, "Normal");
        }

        if ((inStyle.flags & ControlFlags.Hover) != 0) {
            DrawBackgroundField(style.hover, "Hover");
        }

        if ((inStyle.flags & ControlFlags.Active) != 0) {
            DrawBackgroundField(style.active, "Active");
        }

        if ((inStyle.flags & ControlFlags.Focused) != 0) {
            DrawBackgroundField(style.focused, "Focused");
        }
    ...
    T ObjectField<T>(T obj, string title) where T : Object {
        GUILayout.BeginHorizontal();
        GUILayout.Label(title, GUILayout.Width(sLabelWitdh));
    ...
    Enum EnumField(Enum enumType, string title) {
        GUILayout.BeginHorizontal();
        GUILayout.Label(title, GUILayout.Width(sLabelWitdh));

        Enum newNumber = EditorGUILayout.EnumPopup(enumType);
