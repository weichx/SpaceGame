...
namespace Rotorz.ReorderableList {
	...
	public class ReorderableListControl {
		...
		public static void DrawControlFromState(IReorderableListAdapter adapter, DrawEmpty drawEmpty, ReorderableListFlags flags) {
			...
			control.Flags = flags;
			control.Draw(controlID, adapter, drawEmpty);
		}
		...
		/// <param name="drawEmpty">Delegate for drawing empty list.</param>
		/// <param name="flags">Optional flags to pass into list field.</param>
		public static void DrawControlFromState(Rect position, IReorderableListAdapter adapter, DrawEmptyAbsolute drawEmpty, ReorderableListFlags flags) {
			int controlID = GetReorderableListControlID();

			var control = GUIUtility.GetStateObject(typeof(ReorderableListControl), controlID) as ReorderableListControl;
			control.Flags = flags;
			control.Draw(position, controlID, adapter, drawEmpty);
		}
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		private void PrepareState(int controlID, IReorderableListAdapter adapter) {
			_controlID = controlID;
			_visibleRect = GUIHelper.VisibleRect();

			if ((Flags & ReorderableListFlags.ShowIndices) != 0)
				_indexLabelWidth = CountDigits(adapter.Count) * 8 + 8;
			else
				_indexLabelWidth = 0;
		...
		/// </summary>
		/// <param name="adapter">Reorderable list adaptor.</param>
		private void AcceptReorderDrag(IReorderableListAdapter adapter) {
			try {
				// Reorder list as needed!
				s_TargetIndex = Mathf.Clamp(s_TargetIndex, 0, adapter.Count + 1);
				if (s_TargetIndex != s_AnchorIndex && s_TargetIndex != s_AnchorIndex + 1)
					MoveItem(adapter, s_AnchorIndex, s_TargetIndex);
			}
			finally {
		...
		private void DrawListItem(Rect position, IReorderableListAdapter adapter, int itemIndex) {
			...
			bool isDraggable = _allowReordering && adapter.CanDrag(itemIndex);
			...
			try {
				...
				if (isRepainting && isVisible) {
					...
					adapter.DrawItemBackground(backgroundPosition, itemIndex);
				...
				adapter.DrawItem(itemContentPosition, itemIndex);

				if (EditorGUI.EndChangeCheck())
					ReorderableListGUI.IndexOfChangedItem = itemIndex;

				// Draw remove button?
				if (HasRemoveButtons && adapter.CanRemove(itemIndex)) {
				...
				if ((Flags & ReorderableListFlags.DisableContextMenu) == 0) {
					if (Event.current.GetTypeForControl(_controlID) == EventType.ContextClick && position.Contains(Event.current.mousePosition)) {
						ShowContextMenu(itemIndex, adapter);
						Event.current.Use();
					}
				}
			}
		...
		private void DrawFloatingListItem(IReorderableListAdapter adapter, float targetSlotPosition) {
			...
			DrawListItem(s_DragItemPosition, adapter, s_AnchorIndex);
		}
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		private void DrawListContainerAndItems(Rect position, IReorderableListAdapter adapter) {
			...
			switch (eventType) {
				...
				case EventType.ExecuteCommand:
					if (s_ContextControlID == _controlID) {
						...
						try {
							DoCommand(s_ContextCommandName, itemIndex, adapter);
							Event.current.Use();
						}
						finally {
			...
			for (int i = 0; i < count; ++i) {
				...
				if (_tracking) {
					...
					itemPosition.height = adapter.GetItemHeight(i) + 4;
				...
				// Draw list item.
				DrawListItem(itemPosition, adapter, i);

				// Did list count change (i.e. item removed)?
				if (adapter.Count < count) {
					...
					switch (eventType) {
						case EventType.MouseDown:
							if (GUI.enabled && itemPosition.Contains(mousePosition)) {
								...
								if (_allowReordering && adapter.CanDrag(i) && Event.current.button == 0) {
...
//*/
			...
			if (IsTrackingControl(_controlID)) {
				...
				DrawFloatingListItem(adapter, targetSlotPosition);
/* DEBUG
...
//*/
		...
		private void HandleDropInsertion(Rect position, IReorderableListAdapter adapter) {
			var target = adapter as IReorderableListDropTarget;
		...
		/// <param name="position">Position of list control in GUI.</param>
		/// <param name="adapter">Reorderable list adaptor.</param>
		private void DrawFooterControls(Rect position, IReorderableListAdapter adapter) {
			if (HasFooterControls) {
				...
				if (HasSizeField) {
					...
					DrawSizeFooterControl(sizeFieldPosition, adapter);
				}
				...
				if (HasAddMenuButton) {
					...
					if (GUIHelper.IconButton(menuButtonPosition, true, menuIconNormal, menuIconActive, FooterButtonStyle)) {
						...
						OnAddMenuClicked(new AddMenuClickedEventArgs(adapter, totalAddButtonPosition));
		...
		private void DrawSizeFooterControl(Rect position, IReorderableListAdapter adapter) {
			float restoreLabelWidth = EditorGUIUtility.labelWidth;
			EditorGUIUtility.labelWidth = 60f;

			DrawSizeField(position, adapter);
		...
		private Rect GetListRectWithAutoLayout(IReorderableListAdapter adapter, float padding) {
			float totalHeight;

			// Calculate position of list field using layout engine.
			if (Event.current.type == EventType.Layout) {
				totalHeight = CalculateListHeight(adapter);
		...
		/// </returns>
		private Rect DrawLayoutListField(IReorderableListAdapter adapter, float padding) {
			Rect position = GetListRectWithAutoLayout(adapter, padding);
			...
			try {
				// Draw list as normal.
				adapter.BeginGUI();
				DrawListContainerAndItems(position, adapter);
				HandleDropInsertion(position, adapter);
				adapter.EndGUI();
			}
			finally {
		...
		/// Position of list container area in GUI (excludes footer area).
		/// </returns>
		private Rect DrawLayoutEmptyList(IReorderableListAdapter adapter, DrawEmpty drawEmpty) {
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		/// <param name="drawEmpty">Delegate for drawing empty list.</param>
		private void Draw(int controlID, IReorderableListAdapter adapter, DrawEmpty drawEmpty) {
			FixStyles();
			PrepareState(controlID, adapter);

			Rect position;
			if (adapter.Count > 0)
				position = DrawLayoutListField(adapter, 0f);
			else if (drawEmpty == null)
				position = DrawLayoutListField(adapter, 5f);
			else
				position = DrawLayoutEmptyList(adapter, drawEmpty);

			DrawFooterControls(position, adapter);
		}

		/// <inheritdoc cref="Draw(int, IReorderableListAdapter, DrawEmpty)"/>
		public void Draw(IReorderableListAdapter adapter, DrawEmpty drawEmpty) {
			int controlID = GetReorderableListControlID();
			Draw(controlID, adapter, drawEmpty);
		}

		/// <inheritdoc cref="Draw(int, IReorderableListAdapter, DrawEmpty)"/>
		public void Draw(IReorderableListAdapter adapter) {
			int controlID = GetReorderableListControlID();
			Draw(controlID, adapter, null);
		}
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		/// <param name="drawEmpty">Delegate for drawing empty list.</param>
		private void Draw(Rect position, int controlID, IReorderableListAdapter adapter, DrawEmptyAbsolute drawEmpty) {
			FixStyles();
			PrepareState(controlID, adapter);
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		/// <param name="drawEmpty">Delegate for drawing empty list.</param>
		public void Draw(Rect position, IReorderableListAdapter adapter, DrawEmptyAbsolute drawEmpty) {
			int controlID = GetReorderableListControlID();
			Draw(position, controlID, adapter, drawEmpty);
		}

		/// <inheritdoc cref="Draw(Rect, IReorderableListAdapter, DrawEmptyAbsolute)"/>
		public void Draw(Rect position, IReorderableListAdapter adapter) {
			int controlID = GetReorderableListControlID();
			Draw(position, controlID, adapter, null);
		}
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		public void DrawSizeField(Rect position, GUIContent label, IReorderableListAdapter adapter) {
			...
			if (GUI.GetNameOfFocusedControl() == sizeControlName) {
				if (Event.current.rawType == EventType.KeyDown) {
					switch (Event.current.keyCode) {
						...
						case KeyCode.KeypadEnter:
							ResizeList(adapter, _newSizeInput);
							Event.current.Use();
							break;
					}
				}
				...
				EditorGUI.IntField(position, label, adapter.Count);
				_newSizeInput = adapter.Count;
			}
		}
		...
		/// <param name="label">Prefix label for the control.</param>
		/// <param name="adapter">Reorderable list adaptor.</param>
		public void DrawSizeField(Rect position, string label, IReorderableListAdapter adapter) {
			s_Temp.text = label;
			DrawSizeField(position, s_Temp, adapter);
		}
		...
		/// <param name="position">Position of list control in GUI.</param>
		/// <param name="adapter">Reorderable list adaptor.</param>
		public void DrawSizeField(Rect position, IReorderableListAdapter adapter) {
			DrawSizeField(position, s_SizePrefixLabel, adapter);
		}
		...
		/// <param name="label">Prefix label for the control.</param>
		/// <param name="adapter">Reorderable list adaptor.</param>
		public void DrawSizeField(GUIContent label, IReorderableListAdapter adapter) {
			Rect position = GUILayoutUtility.GetRect(0, EditorGUIUtility.singleLineHeight);
			DrawSizeField(position, label, adapter);
		}
		...
		/// <param name="label">Prefix label for the control.</param>
		/// <param name="adapter">Reorderable list adaptor.</param>
		public void DrawSizeField(string label, IReorderableListAdapter adapter) {
			s_Temp.text = label;
			DrawSizeField(s_Temp, adapter);
		}
		...
		/// </summary>
		/// <param name="adapter">Reorderable list adaptor.</param>
		public void DrawSizeField(IReorderableListAdapter adapter) {
			DrawSizeField(s_SizePrefixLabel, adapter);
		}
		...
		private void ShowContextMenu(int itemIndex, IReorderableListAdapter adapter) {
			GenericMenu menu = new GenericMenu();

			s_ContextControlID = _controlID;
			s_ContextItemIndex = itemIndex;

			AddItemsToMenu(menu, itemIndex, adapter);
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		protected virtual void AddItemsToMenu(GenericMenu menu, int itemIndex, IReorderableListAdapter adapter) {
		...
		/// </returns>
		protected virtual bool HandleCommand(string commandName, int itemIndex, IReorderableListAdapter adapter) {
			switch (commandName) {
				case "Move to Top":
					MoveItem(adapter, itemIndex, 0);
					return true;
				case "Move to Bottom":
					MoveItem(adapter, itemIndex, adapter.Count);
					return true;

				case "Insert Above":
					InsertItem(adapter, itemIndex);
					return true;
				case "Insert Below":
					InsertItem(adapter, itemIndex + 1);
					return true;
				case "Duplicate":
					DuplicateItem(adapter, itemIndex);
					return true;
		...
		/// A value of <c>true</c> if command was known; otherwise <c>false</c>.
		/// </returns>
		public bool DoCommand(string commandName, int itemIndex, IReorderableListAdapter adapter) {
			if (!HandleCommand(s_ContextCommandName, itemIndex, adapter)) {
		...
		/// A value of <c>true</c> if command was known; otherwise <c>false</c>.
		/// </returns>
		public bool DoCommand(GUIContent command, int itemIndex, IReorderableListAdapter adapter) {
			return DoCommand(command.text, itemIndex, adapter);
		}
		...
		/// </returns>
		public float CalculateListHeight(IReorderableListAdapter adapter) {
			FixStyles();
			...
			int count = adapter.Count;
			for (int i = 0; i < count; ++i)
				totalHeight += adapter.GetItemHeight(i);
		...
		protected void MoveItem(IReorderableListAdapter adapter, int sourceIndex, int destIndex) {
			// Raise event before moving item so that the operation can be cancelled.
			var movingEventArgs = new ItemMovingEventArgs(adapter, sourceIndex, destIndex);
			OnItemMoving(movingEventArgs);
			if (!movingEventArgs.Cancel) {
				adapter.Move(sourceIndex, destIndex);

				// Item was actually moved!
				int newIndex = destIndex;
				if (newIndex > sourceIndex)
					--newIndex;
				OnItemMoved(new ItemMovedEventArgs(adapter, sourceIndex, newIndex));
		...
		/// </summary>
		/// <param name="adapter">Reorderable list adaptor.</param>
		protected void AddItem(IReorderableListAdapter adapter) {
			adapter.Add();
			AutoFocusItem(s_ContextControlID, adapter.Count - 1);

			GUI.changed = true;
			ReorderableListGUI.IndexOfChangedItem = -1;

			var args = new ItemInsertedEventArgs(adapter, adapter.Count - 1, false);
			OnItemInserted(args);
		}
		...
		protected void InsertItem(IReorderableListAdapter adapter, int itemIndex) {
			adapter.Insert(itemIndex);
			AutoFocusItem(s_ContextControlID, itemIndex);

			GUI.changed = true;
			ReorderableListGUI.IndexOfChangedItem = -1;

			var args = new ItemInsertedEventArgs(adapter, itemIndex, false);
			OnItemInserted(args);
		}
		...
		/// <param name="adapter">Reorderable list adaptor.</param>
		/// <param name="itemIndex">Zero-based index of item.</param>
		protected void DuplicateItem(IReorderableListAdapter adapter, int itemIndex) {
			adapter.Duplicate(itemIndex);
			AutoFocusItem(s_ContextControlID, itemIndex + 1);

			GUI.changed = true;
			ReorderableListGUI.IndexOfChangedItem = -1;

			var args = new ItemInsertedEventArgs(adapter, itemIndex + 1, true);
			OnItemInserted(args);
		}
		...
		/// </returns>
		protected bool RemoveItem(IReorderableListAdapter adapter, int itemIndex) {
			var args = new ItemRemovingEventArgs(adapter, itemIndex);
			OnItemRemoving(args);
			if (args.Cancel)
				return false;
		...
		/// Returns a value of <c>false</c> if operation was cancelled.
		/// </returns>
		protected bool ClearAll(IReorderableListAdapter adapter) {
			if (adapter.Count == 0)
				return true;

			var args = new ItemRemovingEventArgs(adapter, 0);
			int count = adapter.Count;
			for (int i = 0; i < count; ++i) {
				args.ItemIndex = i;
				OnItemRemoving(args);
		...
		/// Returns a value of <c>false</c> if operation was cancelled.
		/// </returns>
		protected bool ResizeList(IReorderableListAdapter adapter, int newCount) {
			if (newCount < 0) {
				// Do nothing when new count is negative.
				return true;
			}

			int removeCount = Mathf.Max(0, adapter.Count - newCount);
			int addCount = Mathf.Max(0, newCount - adapter.Count);

			while (removeCount-- > 0) {
				if (!RemoveItem(adapter, adapter.Count - 1))
					return false;
			}
			while (addCount-- > 0) {
				AddItem(adapter);
